...
class HomeController extends Controller
{
    ...
    public function get_ongkir($destination, $weight)
    {
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_POSTFIELDS => "origin=331&destination=" . $destination . "&weight=" . $weight . "&courier=jne",
            CURLOPT_HTTPHEADER => array(
    ...
    public function orders()
    {
        if (Auth::guard('webmember')->check()) {
            ...
            $orders = Order::where('id_member', $id_member)->get();
            $payments = Payment::where('id_member', $id_member)->get();
    
            return view('home.orders', compact('orders', 'payments'));
        } else {
            return redirect()->route('login_member')->with('error', 'Please log in to view your orders.');
        }
    }
